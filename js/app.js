...
/* ============================================================
   ...
   getSB/enqueueCloudSync stay as bare wrappers here: the not-yet-
   extracted Notifications code calls getSB() directly to register
   push subscriptions, and saveDB() above calls enqueueCloudSync()
   on every persist.
   ============================================================ */
  ...
  if (result === "granted") {
    ...
    if (_sw && window.PushManager) {
      try {
        ...
        const sb = getSB();
        const currentUser = syncModule.getCurrentUser();
        if (sb && currentUser) {
