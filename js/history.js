...
(() => {
  ...
  const shiftKey = (key, amount) => {
    const d = parseKey(key); d.setDate(d.getDate() + amount); return dayKey(d);
  };
  ...
  function renderRecord() {
    ...
    const protocolLines = Object.entries(completions).map(([id,c]) => {
      const p=(DB.protocols||[]).find(x=>x.id===id); return [p ? p.name : "Protocol", c && c.completed ? "Completed" : "Not completed"];
    });
  ...
  function renderStrip() {
    const keys=[];
    // Keep the selected historical date visible instead of always anchoring the strip to today.
    ...
    const active=$("history-day-strip").querySelector(".active"); if(active) active.scrollIntoView({inline:"center",block:"nearest"});
  }

  function render() {
    renderDayHeader(); renderDayMetrics(); renderRange(); renderRecord(); renderStrip();
  }
  function setDate(key) {
    if (!key || key > dayKey()) key=dayKey();
    selectedKey=key; render(); window.scrollTo({top:0,behavior:"smooth"});
  }
  function openHistory() { showView("view-history"); render(); }
  ...
  $("history-range-row").querySelectorAll("button").forEach(b=>b.addEventListener("click",()=>{
    rangeDays=Number(b.dataset.days); $("history-range-row").querySelectorAll("button").forEach(x=>x.classList.toggle("active",x===b)); renderRange();
  }));

  // Keep history accurate after local and cross-tab data changes.
